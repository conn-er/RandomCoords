...
class RandomCoords:
    ...
    def __generate_coord(self):
        ...
        circle_x = self.center[0]
        circle_y = self.center[1]
        circle_radius = (self.diameter / 2) * conversion
        a = random.random() * 2 * pi
        r_rand = sqrt(random.random())
        x = ((circle_radius / 115) * r_rand) * cos(a) + circle_x
        y = ((circle_radius / 95) * r_rand) * sin(a) + circle_y
    ...
    def __generate_path(self, num_points_range: (int, int), circle_radius_range: (int, int)):
        ...
        for i in range(num_points):
            ...
            next_a = random.random() * 2 * pi
            while abs(next_a - last_a) > pi:  # This while loop prevents the next angle from being more than 180 degrees away from the last angle, which creates smoother paths.
                next_a = random.random() * 2 * pi
            r_rand = sqrt(random.random())
            x = ((circle_radius / 115) * r_rand) * cos(next_a) + circle_x
            y = ((circle_radius / 95) * r_rand) * sin(next_a) + circle_y
            ...
            if self.__haversine(x, y, self.center[0], self.center[1]) > self.diameter / 2:
                break
